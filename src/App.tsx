...
function App() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Hero Section */}
      <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-purple-900 via-purple-700 to-blue-900">
        ...
        >
          ...
          <div className="flex flex-wrap gap-4 justify-center mb-12">
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
              className="px-8 py-4 bg-white text-purple-900 font-semibold rounded-full shadow-xl hover:shadow-2xl transition-all"
            >
              {t('hero.cta.catalog')}
            </motion.a>
            <motion.a
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="px-8 py-4 bg-purple-600 text-white font-semibold rounded-full shadow-xl hover:shadow-2xl transition-all"
            >
              {t('hero.cta.contact')}
            </motion.a>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
            {['quality', 'direct', 'wholesale', 'shipping'].map((badge) => (
              <div key={badge} className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20">
                <p className="text-white font-medium">{t(`hero.badges.${badge}`)}</p>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <section id="products" className="py-24 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {['uruguayQuality', 'cutBase', 'heart', 'premium'].map((product, index) => (
              <motion.div
                key={product}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="h-64 bg-gradient-to-br from-purple-200 to-purple-400" />
                <div className="p-6">
                  <h3 className="text-xl font-display font-bold text-gray-900 mb-2">
                    {t(`products.exceptional.items.${product}.title`)}
                  </h3>
                  <p className="text-gray-600 mb-4">
                    {t(`products.exceptional.items.${product}.description`)}
                  </p>
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="w-full py-3 bg-gradient-to-r from-purple-600 to-purple-800 text-white font-semibold rounded-full hover:shadow-lg transition-all"
                    onClick={() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map((item) => (
              <motion.div
                ...
                viewport={{ once: true }}
                whileHover={{ scale: 1.05 }}
                className="aspect-square rounded-2xl bg-gradient-to-br from-purple-200 to-blue-300 shadow-lg hover:shadow-2xl transition-all cursor-pointer"
              />
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              <p className="text-xl text-gray-300 mb-8">{t('story.subtitle')}</p>
              <p className="text-gray-400 leading-relaxed">{t('story.content')}</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, x: 30 }}
              whileInView={{ opacity: 1, x: 0 }}
            ...
            >
              {['quality', 'ethics', 'service'].map((value) => (
                <div key={value} className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10">
